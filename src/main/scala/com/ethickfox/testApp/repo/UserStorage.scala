...
object UserStorage {
  ...
  def updateUserById(id: Long, firstName: String, lastName: String, birthday: Option[LocalDate], address: String): Future[Int] = {
    db.run(
  ...
  def addUser(firstName: String, lastName: String, birthday: Option[LocalDate], address: String): Future[Int] = {
    db.run(users.map(p => (p.firstName, p.lastName, p.birthday, p.address)) +=
      (firstName: String, lastName: String, birthday: Option[LocalDate], address: String))
  }
}